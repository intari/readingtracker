...
public class ParseEmulationNotLoggedInTest {
    ...
    public void testNotLoggedInEmulationInCorrectMode() {
        assertThat("Parse Platform's emulation is in 'not logged in' mode", ParsePlatformUtils.getParsePlatformMode(), equalTo(ParsePlatformUtils.ParsePlatformMode.TEST_NOT_LOGGED_IN));
    }
    @Test
    public void testNotLoggedInGetCurrentUserReturnsNull() {
        ParseUser currentUser=ParsePlatformUtils.getCurrentParseUser();
        assertNull("Not Logged In ParseUser must be null",currentUser);
